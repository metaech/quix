    ...
    fn try_next(&mut self) -> Result<Frame, IterErr> {
        ...
        Ok(match ty {
            ...
            Type::RESET_STREAM => Frame::ResetStream(ResetStream {
                id: self.bytes.get()?,
                error_code: self.bytes.get_varint()?,
                final_offset: self.bytes.get_varint()?,
            }),
            ...
            Type::MAX_DATA => Frame::MaxData(self.bytes.get_varint()?),
...
impl Datagram {
    pub(crate) fn encode(&self, length: bool, out: &mut BytesMut) {
        ...
        if length {
            ...
            out.put_varint(&VarInt::from_u64(self.data.len() as u64).unwrap()); // <= 8 bytes
        }
